...
const SubCategory = () => {
  ...
  useEffect(() => {
    if (id === null) return; // Do not fetch if no subcategory is selected
console.log("id", id)
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const response = await axios.get(
          `${base_url}/product/get_sub_category/${id}`,
          {
            headers: {
        ...
        if (response.status === 200) {
          setProducts(response.data.data.ProductLists); // Assuming products are in this path
        }
      } catch (error) {
            ...
            <div className="row align-items-center justify-content-lg-between">
              <div className="col-lg-6 col-md-6">
                <h4 className="result_text">
                  Showing 1 to {products.length} of {products.length} total
                </h4>
              </div>
            </div>
          </div>
          ...
          <div className="row mb_50 justify-content-center">
            {products.map((product) => (
              <div
                ...
                key={product.id}
              >
                <div className="ss_product_item" style={{ zIndex: 9999}}>
                  <div className="item_image">
                    <Link to={`/detail/${product.id}`}>
                      <img
                        src={product.product_img_thumbnail} // Directly accesses the first image
                  ...
                  <div className="item_content">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      <ul className="rating_star ul_li clearfix mb-0">
                        {/* Assuming product.rank is available for the star rating */}
                        {[...Array(product.product_rank)].map((_, index) => (
                          <li key={index}>
                            <i className="fas fa-star"></i>
                          </li>
                        ))}
                        {[...Array(5 - product.product_rank)].map(
                          (_, index) => (
                            <li key={index}>
