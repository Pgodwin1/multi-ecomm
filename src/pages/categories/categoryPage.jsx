...
const CategoryPage = ({ productItem }) => {
  const { id } = useParams();
  const [subcCategoryData, setCategoryData] = useState([]);
    ...
    const fetchCategoryData = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setCategoryData(response.data.data.Sub_Categories || []);
          console.log("res", subcCategoryData);
        }
  ...
  // Handle category click to fetch products for the selected subcategory
  const handleCategoryClick = (subCategoryId) => {
    sessionStorage.setItem("selectedSubCategoryId", subCategoryId.id);
    navigate(`/subcategory/${subCategoryId.id}`); // Update the selected subcategory ID, triggering the effect
  };
  ...
  useEffect(() => {
    const fetchAllProducts = async () => {
      try {
        const response = await axios.get(
          `${base_url}/product/get_all_product?page=0`
        );
    ...
    // Check sessionStorage for data on component mount
    const savedProducts = sessionStorage.getItem("allProducts");
    if (savedProducts) {
      // Parse JSON if data exists and set it to state
      setAllProducts(JSON.parse(savedProducts));
      setLoading(false);
    } else {
      // Fetch data only if not in sessionStorage
      fetchAllProducts();
    }
  }, []);
  ...
  return (
    <div className="home_simple_shop">
      ...
      {/* ================================================== */}
      <section className="breadcrum_section custom_breadcrum_section text-center d-flex align-items-center" style={{zIndex: 2}}>
        <div className="container row mt-5 custom_container" style={{ marginTop: "30px"}}>
          {subcCategoryData &&
            subcCategoryData.map((category) => (
              <div
                className="col-lg-2 col-md-4 col-sm-6 mb-4 custom_card"
                key={category.id}
              >
                <Link
                  to={`/sub-category/${category.id}`}
                  className="custo_link"
                >
                    ...
                    <div className="ss_product_item">
                      ...
                      <div className="item_content">
                        <h3 className="item_title">
                          <p href="#!">{product.product_title}</p>
                        </h3>
